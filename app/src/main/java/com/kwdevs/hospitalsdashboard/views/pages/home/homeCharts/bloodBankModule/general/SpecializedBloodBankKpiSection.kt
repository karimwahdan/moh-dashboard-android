...
@Composable
fun SpecializedBloodBankKpiSection(controller:HomeController){
    val context       = LocalContext.current
    val kpiBody       =  remember { mutableStateOf<KpiFilterBody?>(null) }
    val state         by controller.specializedKpiState.observeAsState()
    ...
    val exportState by issuingDepartmentController.specializedExcelState.observeAsState()

    val excelFileName       = remember { mutableStateOf("specialized_kpis") }
            ...
            ColumnContainer {
                ...
                CustomButton(label = SAVE_CHANGES_LABEL,
                    ...
                    if(excelFileName.value!= EMPTY_STRING){
                        val b=kpiBody.value
                        b?.let{
                            issuingDepartmentController.saveKpiExcelFile(KpiType.SPECIALIZED,it)
                        }
                    }
            ...
            is UiState.Success<*> -> {
                ...
                if (r.isSuccessful) {
                    val saved = saveExcelFile(
                        responseBody = r.body()!!,
                        fileName = "specialized_bb_kpi.xlsx"
                    )
            ...
            ColumnContainer {
                ...
                Row(modifier=Modifier.fillMaxWidth(), horizontalArrangement = Arrangement.Center){
                    CustomButton(label="Filter",
                        ...
                        kpiBody.value?.let {controller.getSpecializedCharts(it)}
