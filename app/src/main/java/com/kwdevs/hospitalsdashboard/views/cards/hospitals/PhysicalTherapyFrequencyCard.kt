...
@Composable
fun PhysicalTherapyFrequencyCard(item:PhysicalTherapyFrequency){
    val patients=item.patientsCount
    val sessions=item.sessionsCount
    val day=item.day
    ...
    ColumnContainer {
        Row(modifier=Modifier.fillMaxWidth(), horizontalArrangement = Arrangement.SpaceBetween){
            ...
            Row(modifier=Modifier.padding(5.dp),verticalAlignment = Alignment.CenterVertically){
                Icon(R.drawable.ic_physical_therapy, background = BLUE)

                Label(SESSIONS_LABEL,"${sessions?:0}")
            }
        }
    }
}