...
import com.kwdevs.hospitalsdashboard.views.assets.INSTITUTE_LABEL
...
fun BloodImportCreatePage(navHostController: NavHostController){
    ...
    val mainSources= listOf(Pair(1,INSTITUTE_LABEL),Pair(2, PATIENT_LABEL))
    ...
    val selectedMainSource=remember { mutableStateOf<Pair<Int,String>?>(null) }
                            ...
                            if(day.value!= EMPTY_STRING && year.value!= EMPTY_STRING && month.value!= EMPTY_STRING){
                                ...
                                Row(modifier=Modifier.fillMaxWidth().padding(horizontal = 5.dp)){
                                    Box(modifier=Modifier.fillMaxWidth().weight(1f).padding(horizontal = 5.dp)){
                                        ComboBox(
                                            hasTitle = false,
                                            selectedItem = selectedMainSource,
                                            loadedItems = mainSources,
                                            selectedContent = { CustomInput(selectedMainSource.value?.second?: EMPTY_STRING)}) {
                                ...
                                if(selectedMainSource.value!=null && selectedSource.value!=null){
                                        ...
                                        RecipientType.NBTS->{
                                            Box(modifier=Modifier.fillMaxWidth().padding(10.dp)){
                                                ComboBox(
                                                    ...
                                                    selectedItem = selectedHospital,
                                                    selectedContent = { CustomInput(selectedHospital.value?.name?: SELECT_BLOOD_BANK_LABEL)}
                                                ) {
                                                    Label(it?.name?: EMPTY_STRING)
                                                }
                        ...
                        CustomButton(label = SAVE_CHANGES_LABEL,
                            ...
                            if(quantity.value.trim()!= EMPTY_STRING &&
                                ...
                                item=BloodImport(
                                    ...
                                    byPatient = selectedMainSource.value?.first==2,
