...
import com.kwdevs.hospitalsdashboard.modules.bloodBankModule.subModules.donation.views.cards.BloodDonationStatisticsCard
...
fun DailyBloodCollectionIndexPage(navHostController: NavHostController){
    ...
    Container(
        ...
        if(loading) LoadingScreen(modifier=Modifier.fillMaxSize())
            ...
            if(success){
                VerticalSpacer()
                if(user?.hasInnerModulePermission(CRUD.CREATE,InnerModule.BLOOD_COLLECTION)==true){
