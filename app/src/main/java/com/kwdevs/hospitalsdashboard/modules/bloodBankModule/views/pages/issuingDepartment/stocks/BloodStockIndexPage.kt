...
import com.kwdevs.hospitalsdashboard.modules.bloodBankModule.views.cards.issuingDepartment.DailyBloodStockCardListCard
import com.kwdevs.hospitalsdashboard.modules.bloodBankModule.views.cards.issuingDepartment.DailyPlasmaStockCardListCard
import com.kwdevs.hospitalsdashboard.modules.bloodBankModule.views.cards.issuingDepartment.DailyPlateletsStockCardListCard
...
fun BloodStockIndexPage(navHostController: NavHostController){
    ...
    Container(
        ...
        if(loading) LoadingScreen(modifier=Modifier.fillMaxSize())
            ...
            if(success){
                VerticalSpacer()
                CustomButton(label= ADD_NEW_LABEL, buttonShadowElevation = 6, buttonShape = RectangleShape, enabledBackgroundColor = BLUE) { navHostController.navigate(BloodStockCreateRoute.route) }
                PaginationContainer(
                        ...
                        Column(modifier=Modifier.fillMaxSize()) {
                            LazyColumn(modifier= Modifier.fillMaxSize().weight(1f)) {
                                item{
                                    ...
                                    Label(items[0].entryDate?:"", fontWeight = FontWeight.Bold)
...
fun groupBySixHourBlocks(entries: List<DailyBloodStock>): Map<String, List<DailyBloodStock>> {
    ...
    return entries.groupBy { entry ->
        val blockStart=entry.timeBlock?:""
