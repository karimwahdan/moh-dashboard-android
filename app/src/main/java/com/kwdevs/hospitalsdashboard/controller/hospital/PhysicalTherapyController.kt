...
class PhysicalTherapyController : ViewModel() {

    private val api = Callers().physicalTherapyApi()
    ...
    private val paginationData = MutableLiveData<UiState<ApiResponse<PaginationData<PhysicalTherapyFrequency>>>>()
    val paginationState: LiveData<UiState<ApiResponse<PaginationData<PhysicalTherapyFrequency>>>> get() = paginationData

    private val datum = MutableLiveData<UiState<PhysicalTherapyFrequencySingleResponse>>()
    val singleState: LiveData<UiState<PhysicalTherapyFrequencySingleResponse>> get() = datum
    ...
    fun storeNormal(physicalTherapyFrequencyBody: PhysicalTherapyFrequencyBody) {
        viewModelScope.launch(Dispatchers.IO) {
            try {
                withContext(Dispatchers.Main) {datum.value = UiState.Reload}
                val response = api.storeNormal(physicalTherapyFrequencyBody)
