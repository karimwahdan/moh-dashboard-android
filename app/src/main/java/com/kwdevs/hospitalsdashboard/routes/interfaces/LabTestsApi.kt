...
interface LabTestsApi {

    @GET("$HOSPITALS_PREFIX/$LAB_TESTS_PREFIX/$INDEX_PREFIX/by-hospital")
    suspend fun indexByHospital(@Query("page")page:Int,@Query("hospitalId")hospitalId:Int):ApiResponse<PaginationData<LabTestFrequency>>

    @POST("$HOSPITALS_PREFIX/$LAB_TESTS_PREFIX/$STORE_PREFIX/normal")
    suspend fun storeNormal(@Body body: LabTestFrequencyBody):LabTestFrequencySingleResponse
