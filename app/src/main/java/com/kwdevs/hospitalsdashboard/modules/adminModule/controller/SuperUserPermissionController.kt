...
class SuperUserPermissionController: ViewModel() {
    private val api = AdminModuleCallers().superUserPermissionsApi()
    ...
    fun storeSuperUserRole(input: RoleBody){

        viewModelScope.launch(Dispatchers.IO) {
            try {
                withContext(Dispatchers.Main) { singleRole.value= UiState.Loading }
                val response = api.storeSuperUserRole(input)
    ...
    fun updateSuperUserRole(input: RoleBody){
    ...
    fun storeSingleSuperUserPermission(input: PermissionBody){
        viewModelScope.launch(Dispatchers.IO) {
            try {
    ...
    fun updateSingleSuperUserPermission(input: PermissionBody){
        viewModelScope.launch(Dispatchers.IO) {
            try {
                withContext(Dispatchers.Main) { singlePermission.value=UiState.Loading }
