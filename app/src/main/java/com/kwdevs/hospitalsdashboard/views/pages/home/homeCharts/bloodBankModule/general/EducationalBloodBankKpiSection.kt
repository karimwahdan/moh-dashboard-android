...
@Composable
fun EducationalBloodBankKpiSection(controller:HomeController){
    val context       = LocalContext.current
    val kpiBody       =  remember { mutableStateOf<KpiFilterBody?>(null) }
    val state         by controller.educationalKpiState.observeAsState()
    ...
    val exportState by issuingDepartmentController.educationalExcelState.observeAsState()
            ...
            is UiState.Success<*> -> {
                ...
                if (r.isSuccessful) {
                    val saved = saveExcelFile(
                        responseBody = r.body()!!,
                        fileName = "educational_bb_kpi.xlsx"
                    )
    ...
    val excelFileName       = remember { mutableStateOf("educational_kpis") }
            ...
            ColumnContainer {
                ...
                CustomButton(label = SAVE_CHANGES_LABEL,
                    ...
                    if(excelFileName.value!= EMPTY_STRING){
                        val b=kpiBody.value
                        b?.let{
                            issuingDepartmentController.saveKpiExcelFile(KpiType.EDUCATIONAL,it)
                        }
                    }
            ...
            ColumnContainer {
                ...
                Row(modifier=Modifier.fillMaxWidth(), horizontalArrangement = Arrangement.Center){
                    CustomButton(label="Filter",
                        ...
                        kpiBody.value?.let {controller.getEducationalCharts(it)}
