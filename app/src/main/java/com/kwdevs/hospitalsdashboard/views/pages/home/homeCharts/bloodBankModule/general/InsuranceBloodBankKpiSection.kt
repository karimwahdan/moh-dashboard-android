...
@Composable
fun InsuranceBloodBankKpiSection(controller:HomeController){
    val context       = LocalContext.current
    val kpiBody       =  remember { mutableStateOf<KpiFilterBody?>(null) }
    val state         by controller.insuranceKpiState.observeAsState()
    ...
    val exportState by issuingDepartmentController.insuranceExcelState.observeAsState()
            ...
            is UiState.Success<*> -> {
                ...
                if (r.isSuccessful) {
                    val saved = saveExcelFile(
                        responseBody = r.body()!!,
                        fileName = "insurance_bb_kpi.xlsx"
                    )
    ...
    val excelFileName       = remember { mutableStateOf("insurance_kpis") }
            ...
            ColumnContainer {
                ...
                CustomButton(label = SAVE_CHANGES_LABEL,
                    ...
                    if(excelFileName.value!= EMPTY_STRING){
                        val b=kpiBody.value
                        b?.let{
                            issuingDepartmentController.saveKpiExcelFile(KpiType.INSURANCE,it)
                        }
                    }
            ...
            ColumnContainer {
                ...
                Row(modifier=Modifier.fillMaxWidth(), horizontalArrangement = Arrangement.Center){
                    CustomButton(label="Filter",
                        ...
                        kpiBody.value?.let {controller.getInsuranceCharts(it)}
