...
import com.kwdevs.hospitalsdashboard.views.assets.WRITE_FULL_CAMPAIGN_CODE_LABEL
...
fun DailyBloodProcessingCreatePage(navHostController: NavHostController){
    ...
    var validProcessingDate         by remember { mutableStateOf(false) }
    var validBloodType              by remember { mutableStateOf(false) }
    var validBloodCollection        by remember { mutableStateOf(false) }
    var validTotal                  by remember { mutableStateOf(false) }
    ...
    LaunchedEffect(processingDate.value) {
        validProcessingDate=processingDate.value.trim()!=EMPTY_STRING
    }
    LaunchedEffect(selectedBloodType.value) {
        validBloodType=selectedBloodType.value!=null
    }
    LaunchedEffect(selectedBloodCollection.value){
        validBloodCollection=selectedBloodCollection.value!=null
    }
    LaunchedEffect(total.value){validTotal=total.value.trim()!= EMPTY_STRING && total.value.trim().toIntOrNull()!=null}
    ...
    Container(
        ...
        Column(modifier= Modifier.fillMaxSize()){
            LazyColumn(modifier= Modifier.fillMaxSize().weight(1f).padding(start=5.dp,end=5.dp,top=5.dp)) {
                item{
                    ...
                    Label(WRITE_FULL_CAMPAIGN_CODE_LABEL, color = Color.Red, fontSize = 10, paddingStart = 10, paddingEnd = 10, paddingBottom = 5)
                    ...
                    if(selectedBloodCollection.value!=null){
                        ...
                        if(processingDate.value.trim()!=EMPTY_STRING){
                            Row(modifier= Modifier.fillMaxWidth(),
                                ...
                                Label(label = PROCESSING_DATE_LABEL,processingDate.value)
                            }
                        }
                        Row(modifier= Modifier.fillMaxWidth().padding(5.dp),
                            horizontalArrangement = Arrangement.Center){
                            CustomButton(label = SELECT_PROCESSING_DATE_LABEL ,
                                enabledBackgroundColor = ORANGE,
                                onClick = { showProcessingDatePicker.value = !showProcessingDatePicker.value })
                        }
                        if(processingDate.value!= EMPTY_STRING){
                            VerticalSpacer()
                            CustomInput(
                            ...
                            if(total.value.toIntOrNull()!=null){
                                VerticalSpacer(10)
                                Row(modifier= Modifier.fillMaxWidth().padding(horizontal = 5.dp),
                                    ...
                                    CustomButton(label= SAVE_CHANGES_LABEL,
                                        ...
                                        borderColor = if((validProcessingDate && validBloodType && validBloodCollection && validTotal)) GREEN else GRAY,
                                        icon = if((validProcessingDate && validBloodType && validBloodCollection && validTotal)) R.drawable.ic_save_green else R.drawable.ic_cancel_red,
                                        enabled =(validProcessingDate && validBloodType && validBloodCollection && validTotal) ,
                                        ) {
                                        if(processingDate.value.trim()!=EMPTY_STRING &&
            ...
            ColumnContainer {
                ...
                Row(modifier=Modifier.fillMaxWidth().padding(5.dp),
                    ...
                    CustomButton(label = SAVE_CHANGES_LABEL,
                        ...
                        body.value?.let{
                            when(crudType){
                                CrudType.CREATE->controller.storeDailyProcessingNormal(it)
                                CrudType.UPDATE->controller.updateDailyProcessingNormal(it)
                                else-> toast(context, ERROR_CREATING_DATA_LABEL)
                            }
